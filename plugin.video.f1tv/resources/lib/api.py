...
from resources.lib.constants import CONST_BASE_HEADERS, CONST_URLS, CONST_IMAGES, CONST_MAIN_VOD_AR
...
def api_extract_content(data):
    ...
    for row in data:
        if not row['layout'] == 'CONTENT_ITEM':
            try:
        ...
        if len(row['metadata']['pictureUrl']) > 0:
            returnar[id]['image'] = "{image_url}/{image}?w=1920&h=1080&q=HI&o=L".format(image_url=CONST_URLS['image'], image=row['metadata']['pictureUrl'])
        else:
            returnar[id]['image'] = ""
...
def api_vod_download(type, start=0):
    ...
    if type in CONST_MAIN_VOD_AR:
        if type == '804':
            info_url = '{base_url}/2.0/R/ENG/WEB_DASH/ALL/PAGE/SEARCH/VOD/Anonymous/2?orderBy=contractStartDate&sortOrder=asc&filter_Series=W%20Series&filter_orderByDefault=Y&title=W%20Series&pageID=395_804'.format(base_url=CONST_URLS['base'])
        else:
            info_url = '{base_url}/2.0/R/ENG/WEB_DASH/ALL/PAGE/{id}/F1_TV_Pro_Annual/2'.format(base_url=CONST_URLS['base'], id=type)
        ...
        if type == '395':
                ...
                for row2 in row['retrieveItems']['resultObj']['containers']:
                    if check_key(row2, 'metadata') and check_key(row2['metadata'], 'contentSubtype'):
                        if row2['metadata']['contentSubtype'] == 'LIVE' and not row2['id'] in item_ids:                            
                            ...
                            item['type'] = 'event'
                            item['icon'] = '{image_url}/{image}?w=1920&h=1080&q=HI&o=L'.format(image_url=CONST_URLS['image'], image=row2['metadata']['pictureUrl'])
                            item['start'] = ''
            ...
            for row in data['resultObj']['containers']:
                    ...
                    for row2 in row['retrieveItems']['resultObj']['containers']:
                        if row2['layout'] == 'CONTENT_ITEM':
                            ...
                            if check_key(row2['metadata'], 'pictureUrl') and len(row2['metadata']['pictureUrl']) > 0:
                                vodJSON['menu'][row2['id']]['image'] = "{image_url}/{image}?w=1920&h=1080&q=HI&o=L".format(image_url=CONST_URLS['image'], image=row2['metadata']['pictureUrl'])
                            else:
                ...
                if row['layout'] == 'horizontal_thumbnail':
                    ...
                    if check_key(row['metadata'], 'pictureUrl') and len(row['metadata']['pictureUrl']) > 0:
                        vodJSON['menu'][row['id']]['image'] = "{image_url}/{image}?w=1920&h=1080&q=HI&o=L".format(image_url=CONST_URLS['image'], image=row['metadata']['pictureUrl'])
                    elif check_key(row['metadata'], 'imageUrl') and len(row['metadata']['imageUrl']) > 0:
                        vodJSON['menu'][row['id']]['image'] = "{image_url}/{image}?w=1920&h=1080&q=HI&o=L".format(image_url=CONST_URLS['image'], image=row['metadata']['imageUrl'])
                    else:
            ...
            write_file(file=os.path.join('cache', 'menu.json'), data=vodJSON, isJSON=True)
            return vodJSON
    else:
        vodJSON2 = {}
        menu_data = load_file(file=os.path.join('cache', 'menu.json'), isJSON=True)

        if not menu_data["menu"][str(type)] or len(menu_data["menu"][str(type)]['url']) == 0:
            return vodJSON2

        url = "{base_url}{url}".format(base_url=CONST_URLS['base'], url=menu_data["menu"][str(type)]['url'])
