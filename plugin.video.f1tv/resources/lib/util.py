        ...
        for row in playdata['info']['resultObj']['containers']:
            if check_key(row, 'metadata'):
                ...
                if check_key(row['metadata'], 'pictureUrl'):
                    info['image'] = "{image_url}/{image}?w=1920&h=1080&q=HI&o=L".format(image_url=CONST_IMAGE_URL, image=row['metadata']['pictureUrl'])
                    info['image_large'] = "{image_url}/{image}?w=1920&h=1080&q=HI&o=L".format(image_url=CONST_IMAGE_URL, image=row['metadata']['pictureUrl'])
